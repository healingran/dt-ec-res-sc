...
import { nodesUrl, mapNodeId, cpuBarColor } from './nodeApi.js';

const echarts = window.echarts;
if (!echarts) {
    console.error('ECharts 未加载：请确认在 monitor.html 中已引入 echarts.min.js');
}
...
const visualizer = new CityVisualizer('left');
...
const lineOption = {
    ...
    grid: { left: '10%', right: '8%', bottom: '22%', top: '16%', containLabel: true },
...
async function fetchData() {
    ...
    try {
        ...
        if (!Array.isArray(nodes) || nodes.length === 0) {
            setStatus(false, '接口返回 nodes 为空。当前 API：' + url);
            return;
        }
        ...
        setStatus(
            true,
            `第 ${fetchCount} 次拉取 ${t} | ${snap} | API: ${url}（若数值不变请确认后端模拟器与页面端口 CORS）`
        );
        ...
        const hint =
            '请用后端 CORS 已允许的端口打开页面（如 Live Server 5500、8080、3000），' +
            '或在控制台执行：localStorage.setItem("MONITOR_API_BASE","http://127.0.0.1:8000") 后刷新。' +
            (origin ? ` 当前页面 origin：${origin}` : '');
        setStatus(false, `拉取失败：${err && err.message ? err.message : err} | ${url} | ${hint}`);
        console.error(err);
    }
}
